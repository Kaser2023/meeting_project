...
function addVideoStream(video, stream, isScreenSharing = false) {
    video.srcObject = stream;
    video.addEventListener('loadedmetadata', () => {
        video.play();
    });
    ...
    // Create a container for the video and username
    ...
    //Check whether the stream belongs to the current user or others, then extract the right name
    const streamUserName = stream.getTracks().length > 0 && stream.getTracks()[0].getSettings().displayName ? stream.getTracks()[0].getSettings().displayName : "Anonymous";
      userNameElement.textContent = streamUserName;



    videoContainer.appendChild(userNameElement);
    videoContainer.appendChild(video); // Add the video *after* the username


    videoGrid.append(videoContainer); // Append the container
}
