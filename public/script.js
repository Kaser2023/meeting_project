...
socket.on('createMessage', (data) => {
    ...
    const timestampLocal = new Date(timestampUTC).toLocaleTimeString([], {
        hour: '2-digit',
        minute: '2-digit'
    });
...
socket.on('user-connected', (userId, userName) => {
    ...
    // Wait for the new user to send their video stream
    setTimeout(() => {
        ...
        call.on('error', err => {
            console.error(`Error with call for ${userName}:`, err);
        });
    }, 1000); // Delay to ensure readiness
});
...
socket.on('update-participant-list', (participants) => {
    console.log('Participants:', participants.userName); // Debugging
